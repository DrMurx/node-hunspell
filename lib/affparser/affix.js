...
module.exports.parser = {
  ...
  'CIRCUMFIX': function(flag, data) {
    this.emit('circumfix', flag);
  }
...
function afx(type, hash, flag, data) {
  ...
  if (res.length == 1) {
    this.emit('error', "Invalid affix record for flag " + flagh);
    return;
  }

  var wrd = tools.splitBuffer(res[1], [0x2f], 2);
  if (wrd.length > 2) {
    this.emit('error', "Invalid affix record for flag " + flag);
    return;
  }
