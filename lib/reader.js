...
Reader.prototype._loadAffixFile = function(cb) {
  ...
  self._analyzeFile(fileRecord, 
    ...
    function() {
      self._drainAffixBacklog(optionBacklog);
...
Reader.prototype._parseAffixLine = function(data, fileRecord, optionBacklog) {
  ...
  // Defer procession of options which requite a flag until we've finally seen a FLAG
  // option since we need to know,  or a PFX/AFX
  ...
  this._parseAffix(parser, res);
}


Reader.prototype._parseAffix = function(parseFunc, params) {
  // If there are more than 2 parameters on the parser function, first one is a flag
  if (parseFunc.length >= 2) {
    params[0] = this.flagEncoder.read(params[0]);
  }
  parseFunc.apply(this, params);

}


Reader.prototype._drainAffixBacklog = function(optionBacklog) {
  var res, option, parser, params;
  ...
  while (optionBacklog.length) {
    ...
    parser = res[1];
    params = res[2];
    this._parseAffix(parser, params);
  }  
}
